...
class Multiwii(object):
    ...
    ############################################ Flask related variables ##########################################
        ...
        self._lock = threading.Lock()  # Create a lock to syncronize access to hardware from multiple threads.

        self._imu_callback = None  # Initialize callbacks to none
        self._sonar_callback = None
        self._fc_callback = None
        #self._cam_callback = None

        self._imu_thread = threading.Thread(target=self._update_imu, args=(cond1, ))
        self._sonar_thread = threading.Thread(target=self._update_ht, args=(cond2, ))
        self._fc_thread = threading.Thread(target=self._update_fc, args=(cond3, ))
        #self._cam_thread = threading.Thread(target=self._update_cam, args=(cond4, ))      ######  Later... ######

        self.cond1 = threading.Condition()  # condition variable to implement wait() in threads. 
        self.cond2 = threading.Condition()
        self.cond3 = threading.Condition()
        #self.cond4 = threading.Condition()

        self._imu_thread.daemon = True 
        self._sonar_thread.daemon = True 
        self._fc_thread.daemon = True 
    ...
    def recieveIMU(self):
        while True:
            ...
            if self._imu_callback is not None:
                self._imu_callback(self.msp_raw_imu['accx'], self.msp_raw_imu['accy'], self.msp_raw_imu['accz'], self.msp_raw_imu['gyrx'], self.msp_raw_imu['gyry'], self.msp_raw_imu['gyrz'])
    ...
    def on_imu_readings_change(self, callback):
